...
`_same_place` 判定两个地点是否同一：名称完全一致即视为同一；否则以 Haversine 距离计算，距离 ≤ 3 km 视为同一连续锚点，以吸收入口、酒店和交通枢纽的轻微地理编码漂移。短距离连续线仍可在地图上表达实际衔接，但酒店与返程锚点已属于同一地点时不再创建零时长/零距离的“返程接驳”；持久化前也会移除 provider 降级产生的同地无效阶段。返程抵达死线（`return_time`）单独校验，超过 `RETURN_DEADLINE_SILENT_TOLERANCE_MINUTES=15` 分钟的迟到才报告问题。
...
活动排程只把当日路线端点能够代表的景点写入日程，避免模型草案中的高分候选没有进入路线却单独出现在活动卡。酒店按目的地与去重后的代表景点的典型距离选择并尽量跨日复用；每天会在 22:30 前回到住宿基点，返程班次时间紧时优先删减可选游览段而不是跳过酒店衔接。长途自驾没有真实服务区名称时，系统使用道路名与前/中/后段生成可解释候选，并明确提示出发前确认具体站点，不再使用编号占位。

每一日的确定性复核 `review_daily_schedule`（`backend/app/planning/tourism.py`）检查上午/下午/晚间安排、三餐与住宿覆盖、可用时间窗和天气/季节适配，产出核查意见并入 warnings。天气卡统一使用“预测天气参考”表述；预测缺失本身进入 warning/降级，不单独作为 blocker。
...
- 无高德 JSAPI key 或加载失败时，`AmapRouteMap` 整体降级为 `MockRouteMap.vue`（SVG 示意图，仅展示武汉—庐山示例路线与阶段高亮、风险点，不表示真实路况；路线以缓慢循环流动提示其示意性质），并显示「高德地图不可用 · 已切换 Mock 地图」徽标。
