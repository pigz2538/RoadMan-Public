...
- **TripRequest**：从自然语言提取的结构化需求。含 `origin`/`destination`/`start_date`/`end_date`/`departure_time`/`return_time`/`return_before`、`travelers`、`preferences`、`transport_modes`（语义交通偏好，路线执行理解城际班次模式 train/flight/ferry 与高德本地模式）、`special_events`、`must_visit`、`max_days`、`budget`、`max_continuous_drive_minutes` 与 `defaults_applied`。`stay_only_at_destination` 用于阻止一趟语义编辑（如「三天只在九宫山」）复用其它城市或邻近区域的陈旧候选。
