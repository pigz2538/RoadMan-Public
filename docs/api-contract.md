...
- 起终点、交通方式与自然语言时长由需求理解智能体结构化，不以关键词匹配替代语义理解；
- 单次行程支持 1–20 天，7 天及以上通过 `warnings` 返回非阻塞耗时提示；
- 地图能力可用时核验地点是否在境内及路线是否超出单次规划尺度，地图暂时不可用时不据此误拦截；
...
车型目录的搜索与表单接入见 [carinfo-catalog.md](carinfo-catalog.md) 与 [vehicle-public-fallback.md](vehicle-public-fallback.md)。当主目录未覆盖具体车系时，响应 `data.fallback_used=true`，并在条目中返回 `catalog_source`、`source_url`、`detail_source_url`、`specifications`、`specs_missing` 和可选的 `dc_charge_time_hours`；前端应显示来源并提示用户核对年款配置。公开资料没有的字段保持 `null`，不能用演示车型默认值替代。
