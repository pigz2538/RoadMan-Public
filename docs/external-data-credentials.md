...
### Ollama 兼容云 API（`OLLAMA_API_KEY`）

- 落地位置：`backend/app/planning/llm.py` 各智能体（需求提取、需求校验、编辑、POI 策展、目的地研究、POI 排序、POI 适配、特殊事件研究）。
- 用途：语义理解、目的地研究、候选排序/策展/适配、自然语言编辑与附件理解。
- 环境变量：`OLLAMA_API_KEY`、`OLLAMA_MODEL`（默认 `deepseek-v4-flash:0731-cloud`）、`OLLAMA_API_URL`（默认 `https://ollama.com/api/generate`）。
- 是否必需：Agent 能力建议配置；未配置 `OLLAMA_API_KEY` 时需求提取回退到离线结构化解析，只保留硬性结构字段，不做语义猜测。
