...
RoadMan 不是单轮 Prompt 生成器。规划主链包括需求理解、目的地研究、目的地策划、地点候选、适配判断、路线、天气、住宿/餐饮/补能、每日复核、自动修复与持久化。模型负责语义理解和取舍；坐标、时间、能耗、连续性与安全边界由工具和确定性校验器负责。
...
修复循环有次数上限，且不会用模型文本覆盖硬安全结论。失败时保留原版本，不把半成品覆盖为正式行程。
...
在线 Demo 由参赛团队后续部署，本轮不伪造地址。

```text
在线地址：____________________________
演示账号：____________________________
可用时段：____________________________
部署提交：____________________________
健康检查：____________________________/health
```

公网发布前必须完成 HTTPS、强密码、鉴权/租户隔离、密钥托管、限流、备份与隐私告知。当前 Compose 适用于本地或受控局域网 Demo，不应原样暴露数据库、Redis 或后端管理接口到公网。

## 8. 复赛 Demo 建议顺序

1. 首页输入自然语言需求并确认结构化理解。
2. 展示 SSE 渐进规划和工具/智能体阶段。
