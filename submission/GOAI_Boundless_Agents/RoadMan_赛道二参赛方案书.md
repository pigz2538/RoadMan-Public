...
RoadMan 是面向自驾与中短途旅行的多智能体行程工作台。用户用自然语言说出出发地、目的地、日期、同行人、车辆与偏好，系统完成需求核对、目的地锚点校准、目的地研究、真实路线查询、景点与餐饮住宿策展、逐日时间线编排、驾驶休息与补能安排，并在每日复核与最终校验后交付可执行行程。规划是后台持续执行的长任务：多个智能体按职责协作，路线连续性、时间、三餐、住宿、能耗与返程闭环由确定性规则兜底；景点按研究证据、适配性与转场距离选入并与当日路线一致，可修复问题自动重排并再次校验，同地返程锚点不生成零距离移动。用户可继续用自然语言或地图选点增删替换行程，先查看修改预览，确认后统一重排路线与时间，再次校验通过后导出 Markdown、HTML、PDF、PPTX 与长图，所有格式共享同一份冻结快照。系统接入高德地图、Open-Meteo 天气、FlyAI 旅行、OpenTripMap 开放地点与动态多源车型目录，记录来源、调用时间与降级状态，并以测试集、完整旅程验收与调用审计提供可复现证据。RoadMan 不接入车辆控制，不替代实时导航、运营公告或驾驶员判断；其智能体编排、工具适配、状态模型、复核修复与交付模板可复用于房车、骑行、亲子及新能源长途出行等场景。
...
3. 标准化字段，并将同一景区的入口、停车场、游客中心等 provider 变体合并，保留来源与别名；
4. POI 策展智能体判断中文展示名、代表性与研究优先级；
...
- 截图、视频与运行证据提交前必须脱敏；
- 当前已开放受控公网 Demo；首页定位只在用户授权后用于天气，临时失败仅在该浏览器读取最近坐标，不静默回退固定城市，也不上传历史轨迹。升级为生产级多用户服务前仍需补齐账号、租户隔离、数据导出与删除请求和隐私告知。
