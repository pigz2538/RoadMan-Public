---
name: amap-lbs
description: 高德地图综合服务技能，执行 POI 搜索、周边搜索、路径规划（步行/驾车/骑行/公交）、智能旅游规划与热力图可视化。Use when user says "搜美食"、"找酒店"、"天安门在哪"、"西直门周边美食"、"北京南站附近酒店"、"从天安门到故宫怎么走"、"规划驾车路线"、"帮我规划北京一日游"、"杭州西湖游览路线"、"生成热力图"，或表达搜索地点、周边、路线、旅游规划、热力图等意图时。
license: MIT
metadata:
  version: 2.0.1
  author: Gaode / amap
  category: maps-services
...
- **`status` 不为 `"1"`**：地理编码/搜索请求失败。检查 API Key 是否正确、地址是否有效、是否超出调用频率限制。
- **周边搜索坐标缺失**：未先调用地理编码 API。必须先 `geocode/geo?address={位置}` 取得 `location`，再拼接带坐标的搜索链接。
- **热力图链接打不开**：未对 `dataUrl` 做 URL 编码。必须将 `://` → `%3A%2F%2F`、`/` → `%2F` 等转义后再拼接。
- **Key 未配置导致请求被拒**：场景二/四/五/六必须先获取 `AMAP_WEBSERVICE_KEY`。若用户未提供，先提示其到 https://lbs.amap.com 注册获取，再继续。
