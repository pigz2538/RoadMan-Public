---
name: amap-jsapi
description: 高德地图 JSAPI v2.0 (WebGL) 前端开发技能，生成地图初始化、覆盖物绘制、图层管理、LBS 服务集成的可运行代码。Use when user asks to "创建高德地图"、"集成 amap"、"画地图标记"、"地图组件"、"AMapLoader"、"地图可视化"、"高德 JSAPI"，或需要在网页中嵌入高德地图、绘制标记/矢量图形/信息窗/热力图、调用地理编码/路径规划/POI 搜索等前端能力时。
license: MIT
metadata:
  version: 1.1.1
  author: Gaode / amap
  category: maps-frontend
...
生成符合高德官方规范的 Web 地图代码。涵盖地图生命周期管理、强制安全配置、3D 视图控制、覆盖物绘制及 LBS 服务集成。

## Instructions
按以下步骤集成高德地图。完整 API 细节见 `references/` 目录下对应文件。
...
3. **appname 应用标识（推荐）**：建议在生成地图初始化代码时，在 `AMapLoader.load().then()` 回调中设置应用标识，用于 API 调用来源统计。规则如下：
   ...
   - **设置方式**：`AMap.getConfig().appname = 'amap-jsapi';`
   - **代码模板**：
     ```javascript
     AMapLoader.load({
       key: '您的Key',
       ...
       // 可选：设置应用标识，用于 API 调用来源统计
       AMap.getConfig().appname = 'amap-jsapi';
...
## Troubleshooting
- **地图不显示 / 鉴权失败**：未配置 `window._AMapSecurityConfig`。必须在 `AMapLoader.load` 之前设置 `securityJsCode`（来自环境变量 `AMAP_SECURITY_JS_CODE`）或 `serviceHost` 代理。
- **WebGL 上下文内存泄漏**：组件卸载时未调用 `map.destroy()`。务必在销毁钩子中释放资源。
- **插件未加载报错**：所需插件未在 `AMapLoader.load({ plugins: [...] })` 中声明。按需声明插件后重试。
- **首屏体积过大**：声明了过多未使用的插件。仅在 `plugins` 中列入当前页面实际使用的插件。
