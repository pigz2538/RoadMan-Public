...
RoadMan's approach: **put "content suggestions" into an executable workflow**. A one-line requirement is handed to a team of specialized agents that understand, research, curate and orchestrate — while hard rules like route coordinates, time windows, continuous driving, battery margins, meals & lodging, and the return loop are enforced by deterministic code. Re-verification triggers up to three automatic repair passes; any remaining planner checks are delivered with the current draft as pre-departure reminders instead of a blocking failure dialog. When you say "make day 2 a quieter place", the system first shows an impact preview, then recomputes everything after confirmation and re-validates before allowing export.
...
### Public transport and fuel-price fallbacks

When the primary travel-information service has no usable schedule, RoadMan tries the public train endpoint configured by `TRAIN_FALLBACK_URL`. The flight fallback is opt-in with `FLIGHT_FALLBACK_API_KEY`; fuel prices are optional with `OIL_APP_ID` and `OIL_APP_SECRET`. Every fallback keeps its source, real service number and availability status; missing credentials or empty results are reported instead of fabricated. Non-thinking mode is the default (`DEEPSEEK_THINKING=false`, `DEEPSEEK_REASONING_EFFORT=low`); enable deeper reasoning only for a dedicated evaluation run.
