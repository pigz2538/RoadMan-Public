...
test('正常需求预检后进入最终确认而不是空阻塞', async ({ page }) => {
  // This case intentionally exercises the real cloud requirement agent.  Do
  // not make the whole browser suite depend on a developer's expiring key:
  // the API smoke/contract tests cover the unavailable-agent path, while
...
test('阶段 D 真实 Agent 行程可展示地图、阶段和 Markdown', async ({ page }) => {
  ...
  await expect(page.locator('.map-live-badge, .map-fallback-badge')).toBeVisible({ timeout: 30_000 })
...
test('首页在规划前集中询问矛盾与缺失信息', async ({ page }) => {
  ...
  await input.fill(
    '2099-08-02从上海出发跨海去普陀山，2099-08-01返回，下午3点出发到下午3点抵达',
  )
  await page.getByRole('button', { name: '开始规划' }).click()
  ...
  await panel.locator('.preflight-answer').fill('2099-08-03')
  await panel.getByRole('button', { name: /下一个问题|重新检查全部条件/ }).click()
