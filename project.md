...
RoadMan 已形成从「自然语言需求」到「可确认、可追踪、可编辑、可导出的行程」的完整闭环：后端 LangGraph 规划工作流经 ARQ + Redis 异步执行，前端提供首页、规划进度、详情地图、阶段卡片、候选推荐和智能体编辑面板。

已落地的主线能力：

- Trip 数据按 `天 → MovementStage / Activity` 组织，使用 PostgreSQL（生产/Docker）或 SQLite（本地/测试）和 Alembic 迁移。
- `preflight` 先由需求理解智能体提取语义需求、日期/地点/交通约束并进行必要追问；用户确认前不会开始规划。地点与时长意图不使用字符串/城市关键词兜底：智能体不可用或返回非法结构时明确暂停，不向地图发送猜测值。地图工具随后核验境内位置与路线尺度，确定性策略只执行产品边界（单次最多 20 天；7–20 天提示生成可能更久；超出境内或超长距离时要求拆分），不负责猜地点或意图。
- 明确的「出发—抵达」时钟窗口作为硬约束复核；返程目标允许 15 分钟静默误差、半天内只给 warning，超过半天才阻断。
- 跨海只在用户明确写出跨海语义时保留安全约束；轮渡/飞机/桥梁方式不由模型凭空猜定，需用户确认或明示。
- LangGraph 规划工作流执行需求抽取、目的地研究、目的地策划单、路线、POI、天气、补能/服务、每日复核和持久化节点。省份/城市/多目的地先由研究智能体检索必去地标与代表性美食，再由目的地策划智能体按地理片区生成每日主轴，最后交给路线智能体执行；当一句话同时出现行政区父级与明确子城市时，以用户真正要去的子级作为路线锚点，重新规划时若语义锚点变化会清除旧坐标，避免前端标题与地图中心指向不同地点。
- Skill Registry 统一接入地图、天气、旅行信息、开放数据、车型目录和语义模型；每次调用可审计、可缓存并可降级。景点、餐饮和住宿排入行程后再做精确核验，保留来源数、核验时间、营业/门票/预约状态、价格区间、停车与详情链接；旅行信息服务的住宿结果按目的地坐标过滤异地卡片，无库存或字段缺失时保留未知状态。公共交通保留线路与上下车站，火车/飞机保留真实编号、站场、时间、座席、价格和详情链接，不用交通方式名称充当虚构班次。完整契约见 [docs/mobility-and-poi-data-contract.md](docs/mobility-and-poi-data-contract.md)。
- 在线路线优先使用真实道路 geometry。用户未声明交通方式时，跨城与市内都保持自驾；驾车不可用会返回 `ROUTE_UNAVAILABLE`，不会暗中切换飞机、火车、公交或骑行。只有用户明确选择飞机/火车时，跨城才查询真实班次，市内接驳才使用公共交通/步行；前端灰色虚线只表示不可用提示。
- 当前阶段路线高亮，其他路线灰显；地图支持平移/缩放、阶段切换和点选；路线几何跟随地图变换。景点候选先合并同一景区的入口/停车场等变体，再综合研究优先级、适配评分和当前路线距离选取；日程活动只采用当日路线实际经过的候选，避免卡片与地图脱节。
- 规划页左侧日程、右侧行程助理和底部阶段详情可独立折叠；底栏折叠后只保留阶段序号及前后切换，状态写入浏览器本地存储，展开不会重建组件或丢失编辑上下文。规划等待层按需求理解、地点搜索、候选筛选、多智能体协作、上下文、编排和复核逐幕切换，每幕只有一个主视觉，动画节奏不伪装后台进度。
- 每日复核与最终验证由确定性调度和校验器执行，覆盖三餐、住宿、时段、活动冲突、连续驾驶、补能和返程闭环；发现问题时自动重排并循环复核最多 3 轮，仍未消解的校验项统一作为“出发前提醒”随当前草案交付，不再用“规划未完成”阻塞用户查看完整计划。每个目的地游览日会在晚间截止前回到选定酒店；为赶返程班次会先删减可选景点而不跳过酒店，酒店与返程锚点被判为同一地点时不生成零时长/零距离接驳。明确返程到达时刻会在排程前预留道路、补能和驾驶休息缓冲，避免深度驾驶拆分后越过截止时间。长途自驾会按每日最多 9 小时拆成跨天路段，在沿途服务区/酒店插入休息与过夜住宿，次日 08:00 再继续，不会把 24 小时以上驾驶压在第一天。
...
- 车辆管理支持 CRUD，并提供 `carinfo.catalog` 真实车型搜索：主目录与多个公开车型数据源并行补足具体版本，缺失字段保持为空并附来源/估算标记；本地车型字段由用户确认后保存。
...
flowchart LR
    subgraph FE["浏览器 · Vue 3"]
        A["首页：需求录入 / 预检问答 / 历史行程 / 车型管理 / 3D 车辆 / 当前位置天气"]
        B["规划页：SSE 进度 / 地图 / 阶段卡片 / 活动列表 / 智能体面板 / 导出"]
    end
...
→ verify_plan
  ├─ 可修复且未超过 3 轮 → repair_plan → verify_plan
  ├─ 三轮后仍有校验项 → 以 best_effort 交付草案，并列为出发前提醒
  └─ 通过 / best_effort → render_markdown → persist_trip → END
```
...
`repair_plan` 会真正重新执行排程与复核：从去重后的策展候选重建三餐、住宿和景点，候选选择同时考虑研究证据、适配性与相邻路线距离，归一化供应商返回的重叠时间，补齐晚间酒店闭环并移除同地零长度接驳，再把结果交回 `verify_plan` 重新校验。
...
7. 行程逐步持久化；最终复核通过，或三次自动重排后以 `best_effort`（附出发前提醒）交付时，将 Trip 标为 `completed`。只有必填信息尚未补齐或代码执行异常才会暂停/失败。
...
`amap.route` 接受 `preferred_mode` 和 `allowed_fallback_modes`。真实 geometry 优先；无 geometry 的结果不作为道路使用。未声明交通方式时允许模式仅为 `driving`，跨城和市内均不自动改成公交、骑行、火车或飞机。用户明确选择飞机/火车时，跨城班次必须带真实航班号/车次号和站场时刻，市内使用公共交通与短距离步行。所有可用模式失败均返回结构化错误，不生成占位班次。
...
| 天气与首页位置 | `open_meteo.forecast`、`amap.reverse_geocode` | 浏览器授权后按当前坐标查天气并解析标签；定位短暂失败仅使用本机保存的上次位置，无位置时不回退固定城市 |
