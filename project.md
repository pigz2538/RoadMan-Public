...
- 修复城市目的地被附近同名餐饮 POI 覆盖的问题：高德地理编码现在保留行政级别，
  “武汉 → 北京”不会再触发武汉附近 POI 歧义纠正并变成“北京片皮烤鸭”；短景点名
  （如乌镇）仍保留出发点附近二次消歧。显式“周日晚八点前回来”按抵达截止时间
  编排返程，并在道路耗时超出截止时间时阻断完成状态、给出可执行调整原因。
- 修复长途补能/休息点落在同一道路点列时的排序崩溃；目的地接驳阶段现在使用成功路线的单调序号，返程接驳不会排到后续景点之前。真实“武汉→北京”回归已验证目的地保持为北京，时间窗不可行时返回明确的 `RETURN_DEADLINE_UNACHIEVABLE`，不再落到“北京片皮烤鸭”。
- 天气服务没有逐时数据时只记录“天气数据暂缺，已继续规划，出发前复核”警告，不再阻断本身可执行的行程；若同时存在返程截止、住宿缺失等真正阻断项，校验与前端弹窗会优先展示阻断原因。
...
- 后端 pytest：91 项通过，1 个真实接口集成用例默认跳过（包含目的地研究名胜优先级、片区聚类、跨天覆盖、未覆盖提示、城市目的地消歧、返程截止时间、长途同点服务 POI 和天气降级回归）。
