...
- 规划容错：旅游编排会为 Agent 漏出的日期补齐稳定 `day_id`，避免单个缺失字段中断整条行程；Worker 对内部字段错误显示可操作的中文原因，不再把裸 `KeyError` 暴露给用户。云端 Requirement Agent 暂时不可达时，显式的情侣/夫妻语义不会静默回退成 1 人，重试规划也会覆盖旧的 `travelers=1` 默认值。
