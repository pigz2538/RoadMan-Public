...
- 最新天气与地图修复（2026-08-07）：天气以 24 小时为可信边界，远期阶段改用目的地当天天气参考，不再输出远期逐时复核估算；地图会在阶段/活动异步补齐后重新绘制，为每个阶段起终点和所有接驳、步行、骑行、餐饮、住宿、景点活动保留可点击标记，缺少地点坐标时从已保存道路点列补齐。
- 景点预约核查增强：FlyAI 门票候选的票名与票期会保留到活动，未能确认预约规则的景点统一标注“预约规则待核查”，并显示官方预约、实名和分时入园核查说明。
...
- 跨城晚到保护：允许火车/飞机等候选会综合比较到达时间与舒适度；18:00 后抵达不再生成本地观光接驳，返程发车后也不再填充景点，景点活动硬限制在当日 07:00–22:00。
- 每个景点、餐饮和住宿活动都会写入预约状态、预约核查说明、风险等级、风险标签与风险说明；详情页、导出 HTML/PDF/PPTX/长图均展示这些字段。青年公寓、学生公寓等非舒适住宿也会被过滤。
...
- 跨城交通统一接入 FlyAI：新增 `flyai.flight` 航班适配器与 `flyai.ferry` 轮船/渡轮语义检索适配器，并与既有 `flyai.train` 合并为统一班次候选。默认仍先走高德真实驾车路线；只有用户明确选择火车/飞机/轮船、语义约束要求跨海，或驾车路线不可用时才依次尝试班次回退。航班保留航班号、机场、时刻、票价与来源；轮船结果因供应商暂无统一时刻表而标记为“估算班次”，要求出发前按船公司确认。
...
- 天气按“未来 24 小时”边界处理：24 小时内使用预计抵达时段的小时预报，超过 24 小时直接显示目的地当天天气参考，不把远期估算伪装成逐时预报；天气服务暂不可用时只记录“当前天气数据暂不可用，已按基础风险继续规划”警告，不阻断本身可执行的行程。
- 跨城交通现在会比较所有用户允许的火车/航班/轮船班次，优先白天抵达；跨午夜的首日接驳与景点会被移出原日历日，景点硬性限制在当天 07:00–22:00，避免出现凌晨三点游览天安门等不可执行安排。若只能选择夜间班次，会保留明确的提示并把景点安排到抵达后的白天。
- 每个景点、餐饮和住宿活动新增预约状态（需预约/建议预约/无需预约/待核查）及风险等级、风险标签和核查说明；详情页活动清单与 HTML/PDF/PPTX/长图使用的报告卡都会展示这些信息，来源不足、营业时间未确认、季节/天气适配不足和非舒适时段会明确标出。
...
- 天气按每个阶段的预计到达坐标与时刻匹配 Open-Meteo 小时预报；未来超过 24 小时
  时改用目的地当天天气参考，避免展示不可靠的远期逐时估算。
...
- 后端核心规划回归：44 项通过；全量套件 94 项通过、1 个真实接口集成用例默认跳过，另有 3 项依赖测试环境云端模型/本地密钥/附件解析的基线失败（不影响本轮跨日排程与预约风险改动）。
